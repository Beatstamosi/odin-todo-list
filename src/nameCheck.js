...
export function checkProjectNameExists() {
  ...
  projectNameInput.addEventListener("keyup", () => {
    ...
    let projectExists = existingProjectNames.some(
      (project) => project.name === projectNameInput.value,
    );
...
export function checkTaskNameInsideProjectExists() {
  ...
  const checkTaskName = function () {
    let existingProjectNames = toDoList.retrieveFromLocalStorage();
    let projectExists = existingProjectNames.find(
      (project) => project.name === select.value,
    );

    if (projectExists) {
      taskExistsInsideProject = projectExists.tasks.some(
        (task) => task.name === taskNameInput.value,
      );
    }
